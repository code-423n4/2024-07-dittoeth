...
contract MultiAssetForkTest is MultiAssetForkTestHelper {
    ...
    function checkEthEscrowed(address user) internal view {
        uint256 amount;
        if (user == sender) {
    ...
    function checkErcEscrowed(address user) internal view {
        uint256 amount;
        if (user == sender) {
    ...
    function testFork_MultiAsset() public {
        ...
        checkErcDebt();
        skip(skipTime); // legitimize yield after partial exit
        ...
        assertEq(diamond.getShortRecords(_cxau, receiver)[0].updatedAt, diamond.getOffsetTime());
...
contract BaseOracleChainlinkRevert is MultiAssetForkTestHelper {
    ...
    function testFork_BaseOracleTryCatchDUSD() public view {
        assertEq(diamond.getOracleAssetPrice(_dusd), twapDusdPrice.inv());
    }

    function testFork_BaseOracleTryCatchMultiAsset() public view {
    ...
    function testFork_Revert_BaseOracleTryCatchMultiAsset() public {
        ...
        // @dev As of 4/24/24 there is no asset other than dusd
