...
contract ChainlinkForkTest is ConstantsTest {
    ...
    function getOracleData() private view returns (uint80, uint256, uint256, uint256, uint80) {
        (uint80 roundID, int256 basePrice, uint256 startedAt, uint256 timeStamp, uint80 answeredInRound) =
            baseOracle.latestRoundData();
        ...
        // emit LogNamedUint256("roundID", roundID);
        if (basePrice == 0) revert("is 0");
        return (roundID, (uint256(basePrice * ORACLE_DECIMALS)).inv(), startedAt, timeStamp, answeredInRound);
    }

    function getHistoricalData(uint80 roundId) public view returns (uint80, uint256, uint256, uint256, uint80) {
        (uint80 roundID, int256 price, uint256 startedAt, uint256 timeStamp, uint80 answeredInRound) =
            baseOracle.getRoundData(roundId);
        require(timeStamp > 0, "Round not complete");
        if (price == 0) revert("is 0");
        return (roundID, (uint256(price * ORACLE_DECIMALS)).inv(), startedAt, timeStamp, answeredInRound);
    }

    function testFork_ChainlinkLatest() public view {
    ...
    function testFork_ChainlinkHistorical() public view {
    ...
    function testFork_ChainlinkCurrentRound() public view {
        (
