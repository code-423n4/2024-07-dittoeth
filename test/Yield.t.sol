...
contract YieldTest is OBFixture {
    ...
    function test_view_getTithe() public view {
    ...
    function test_CanInitializeState() public view {
        assertEq(diamond.getDethTotal(vault), 4000 ether);
        assertEq(diamond.getVaultStruct(vault).dethTotal, 4000 ether);
    ...
    function test_ExitShortYieldDisbursement() public {
        ...
        // @dev skipping to allow shorters to be eligible for yield
        skip(yieldEligibleTime);
    ...
    function test_ProposeRedemptionYieldDisbursement() public {
        // Create shorts and generate yield
        fundLimitBid(DEFAULT_PRICE, DEFAULT_AMOUNT * 2, extra);
        ...
        assertEq(diamond.getVaultUserStruct(vault, receiver).ethEscrowed, 1000 ether);
        ...
        proposalInputs[0] = MTypes.ProposalInput({shorter: receiver, shortId: C.SHORT_STARTING_ID, shortOrderId: 0});
        ...
        skip(yieldEligibleTime);
        vm.prank(redeemer);
        proposeRedemption(proposalInputs, DEFAULT_AMOUNT * 2);

        // claim to get remaining collateral
        skip(5523 seconds); //logged
        vm.prank(extra);
    ...
    function checkTappDidNotReceiveYield() public view {
        assertEq(diamond.getVaultUserStruct(vault, tapp).ethEscrowed, 0);
    }

    function checkTappReceivedYield() public view {
    ...
    function test_DittoRewardPenalty() public {
        ...
        uint256 dittoRewardPenalized = dittoRewardShortersTotal * 1 / 9 - 1; // represents /9
