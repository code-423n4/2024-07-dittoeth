...
contract ERC721Test is OBFixture {
    ...
    function checkNFTBeforeTransfer() public view {
        ...
        assertEq(shortRecord.tokenId, 1);
        assertEq(shortRecord.collateral, col);
        assertEq(shortRecord.ercDebt, DEFAULT_AMOUNT);
        assertSR(shortRecord.status, SR.FullyFilled);
    }

    function checkNFTAfterTransfer() public view {
        STypes.NFT memory nft = diamond.getNFT(1);
    ...
    function test_Revert_TransferFrom_ERC721NonexistentToken_NFTBurned() public {
        createShortAndMintNFT();
        ...
        STypes.NFT memory nft = diamond.getNFT(1);
        // NFT deleted because SR was deleted (exitShort)
    ...
    function test_Revert_TransferFrom_CannotTransferSRToTapp() public {
        createShortAndMintNFT();
        vm.prank(sender);

        vm.expectRevert(Errors.CannotTransferSRToTapp.selector);
        diamond.transferFrom(sender, tapp, 1);
    }
    ...
    function test_CombineShort_TwoShortsTwoNFTs() public {
        ...
        assertEq(diamond.ownerOf(2), address(0));
    }
    ...
    //Partial Fill
    function test_Revert_MintNFT_CannotMintNFTForPartiallyFilledSR() public {
        fundLimitShortOpt(DEFAULT_PRICE, DEFAULT_AMOUNT, sender);
        fundLimitBidOpt(DEFAULT_PRICE, DEFAULT_AMOUNT / 2, receiver);

        vm.prank(sender);
        vm.expectRevert(Errors.CannotMintNFTForPartiallyFilledSR.selector);
        diamond.mintNFT(asset, C.SHORT_STARTING_ID);
    }
    ...
    //Code4Rena Audit
    //https://github.com/code-423n4/2024-03-dittoeth-findings/issues/174
    ...
    function test_PoCNFTTransferPossibleWithReusedID() public {
        ...
        STypes.ShortRecord memory firstSR = diamond.getShortRecord(asset, sender, C.SHORT_STARTING_ID);
        assertEq(firstSR.collateral, 7500000000000000000);
        assertEq(firstSR.ercDebt, 5000000000000000000000);
        ...
        // @dev At this point, owners and shorterRecordId are deleted after exit
        STypes.NFT memory nftAfterExit = diamond.getNFT(1);
        assertNotEq(nftAfterExit.owner, nftBefore.owner);
        assertNotEq(nftBefore.shortRecordId, nftAfterExit.shortRecordId);

        fundLimitBidOpt(DEFAULT_PRICE, DEFAULT_AMOUNT * 2, receiver);
        fundLimitShortOpt(DEFAULT_PRICE, DEFAULT_AMOUNT * 2, sender); // shortRecordId reused

        STypes.ShortRecord memory secondSR = diamond.getShortRecord(asset, sender, C.SHORT_STARTING_ID);
        assertEq(secondSR.collateral, 15000000000000000000);
        assertEq(secondSR.ercDebt, 10000000000000000000000);

        assertEq(diamond.getShortRecord(asset, sender, C.SHORT_STARTING_ID).tokenId, 0);

        vm.prank(sender);
        diamond.mintNFT(asset, C.SHORT_STARTING_ID); // mint NFT again with new SR (NFT tokenId #2)

        assertEq(diamond.getShortRecord(asset, sender, C.SHORT_STARTING_ID).tokenId, 2);

        // @dev Confirm that the NFT no longer shares the owner
        STypes.NFT memory nftAfterNewSRCreated = diamond.getNFT(1);
        assertNotEq(nftAfterNewSRCreated.owner, nftBefore.owner);

        assertEq(diamond.getShortRecord(asset, attacker, C.SHORT_STARTING_ID).collateral, 0); // attacker has no SR
        vm.prank(attacker);
        // @dev Added this line to make sure bug has been fixed
        vm.expectRevert(abi.encodeWithSelector(Errors.ERC721InsufficientApproval.selector, attacker, 1));
        diamond.transferFrom(sender, attacker, 1); // attacker can no longer transfer NFT, and move new SR
    }
    ...
    function test_AttackCancelsSomeoneElsesShortOrderViaTransferFrom() external {
    ...
    function test_AttackCancelsSomeoneElsesShortOrderViaTransferFrom_MoreComplex() external {
    ...
    function test_AttackCancelsSomeoneElsesShortOrderViaTransferFrom_Indefinite() external {
        // Random addresses, can be indefinite
        address[3] memory attackerAddresses = [
            0x5a9a88bD94c8410243B0c04018811b8cA4D09D55,
            0x12e9757fB4a2990aDaf10A6ca8C7085C06cF7173,
    ...
    function test_OneSROneNFT() public {
        fundLimitShort(DEFAULT_PRICE, DEFAULT_AMOUNT, sender);
        fundMarketBid(DEFAULT_PRICE, DEFAULT_AMOUNT, sender);
        vm.prank(sender);
        diamond.mintNFT(asset, C.SHORT_STARTING_ID);
        ...
        //delete SR and burn NFT
        exitShortErcEscrowed(C.SHORT_STARTING_ID, DEFAULT_AMOUNT, sender);

        //make another short (reuse id)
        fundLimitShort(DEFAULT_PRICE, DEFAULT_AMOUNT, sender);
        fundMarketBid(DEFAULT_PRICE, DEFAULT_AMOUNT, sender);
        vm.prank(sender);
        diamond.mintNFT(asset, C.SHORT_STARTING_ID);

        // @dev The NFTs no LONGER point to the same owner/SR
        nft = diamond.getNFT(1);
        assertEq(nft.owner, address(0));
        assertEq(nft.shortRecordId, 0);
        nft = diamond.getNFT(2);
        assertEq(nft.owner, sender);
        assertEq(nft.shortRecordId, C.SHORT_STARTING_ID);
    }
}
