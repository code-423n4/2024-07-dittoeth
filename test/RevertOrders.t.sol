...
contract RevertOrdersTest is OBFixture {
    ...
    function test_RevertIf_NotContract() public {
        ...
        vm.expectRevert(Errors.NotDiamond.selector);
        IDiamond(payable(address(_diamond))).createForcedBid(sender, asset, 1 ether, 2 ether, shortHintArrayStorage);

        vm.expectRevert(Errors.NotDiamond.selector);
        vm.prank(sender);
        diamond._cancelAsk(asset, C.STARTING_ID);

        vm.expectRevert(Errors.NotDiamond.selector);
        IDiamond(payable(address(_diamond)))._cancelAsk(asset, C.STARTING_ID);

        vm.expectRevert(Errors.NotDiamond.selector);
        vm.prank(sender);
        diamond._cancelShort(asset, C.STARTING_ID);

        vm.expectRevert(Errors.NotDiamond.selector);
        IDiamond(payable(address(_diamond)))._cancelShort(asset, C.STARTING_ID);
    }
    ...
    function test_RevertIf_NotEnoughDeposit_Short() public {
        uint88 minAskEth = DEFAULT_PRICE.mulU88(DEFAULT_AMOUNT).mulU88(LibOrders.convertCR(initialCR));
    ...
    function test_RevertIf_NotEnoughDeposit_MarketAsk() public {
        ...
        vm.expectRevert(Errors.InsufficientERCEscrowed.selector);
        diamond.createAsk(asset, DEFAULT_PRICE, DEFAULT_AMOUNT, C.MARKET_ORDER, badOrderHintArray);
    }
    ...
    function test_RevertIf_PriceOrQuantity0() public {
        ...
        vm.expectRevert(Errors.OrderUnderMinimumSize.selector);
        diamond.createLimitShort(asset, 1, 0, badOrderHintArray, shortHintArrayStorage, initialCR);

        vm.expectRevert(Errors.OrderUnderMinimumSize.selector);
        diamond.createLimitShort(asset, 0, 1, badOrderHintArray, shortHintArrayStorage, initialCR);
    ...
    function test_RevertIf_PriceOrQuantity0_Bid() public {
        vm.expectRevert(Errors.OrderUnderMinimumSize.selector);
        diamond.createBid(asset, 0, 0, C.LIMIT_ORDER, badOrderHintArray, shortHintArrayStorage);
    }
    ...
    function test_RevertIf_PriceOrQuantityInvalid_MarketAsk() public {
        ...
        vm.expectRevert(Errors.OrderUnderMinimumSize.selector);
        diamond.createAsk(asset, 0, DEFAULT_AMOUNT, C.MARKET_ORDER, badOrderHintArray);
    }
    ...
    function test_RevertIf_PriceOrQuantityInvalid_MarketBid() public {
        ...
        vm.expectRevert(Errors.OrderUnderMinimumSize.selector);
        diamond.createBid(asset, DEFAULT_PRICE, 0, C.MARKET_ORDER, badOrderHintArray, shortHintArrayStorage);

        vm.expectRevert(Errors.OrderUnderMinimumSize.selector);
        diamond.createBid(asset, 0, DEFAULT_AMOUNT, C.MARKET_ORDER, badOrderHintArray, shortHintArrayStorage);
    }
    ...
    function test_RevertIf_UnderMinSize_Ask() public {
        ...
        vm.expectRevert(Errors.OrderUnderMinimumSize.selector);
        diamond.createAsk(asset, DEFAULT_PRICE, 0.39999999 ether, C.MARKET_ORDER, badOrderHintArray);
    }
    ...
    function test_RevertIf_UnderMinSize_Bid() public {
        ...
        vm.expectRevert(Errors.OrderUnderMinimumSize.selector);
        diamond.createBid(asset, DEFAULT_PRICE, 0.39999999 ether, C.MARKET_ORDER, badOrderHintArray, shortHintArrayStorage);
    }
}
