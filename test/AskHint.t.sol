...
import {U80} from "contracts/libraries/PRBMathHelper.sol";
...
contract AskHintTest is OBFixture {
    ...
    function assertEqHint(STypes.Order[] memory asks) public pure {
    ...
    // @dev This also tests https://github.com/code-423n4/2024-03-dittoeth-findings/issues/132
    ...
    function test_findOrderHintId() public {
        uint256 numShorts = 100;

        // @dev Match a short to trigger anyOrderHintPrevMatched bool in findOrderHintId()
        fundLimitShortOpt(DEFAULT_PRICE, DEFAULT_AMOUNT, sender);
        fundLimitBidOpt(DEFAULT_PRICE, DEFAULT_AMOUNT, sender);

        //To set startingShortId far away, not HEAD
        for (uint256 i = 0; i < numShorts; i++) {
            fundLimitShortOpt(DEFAULT_PRICE - uint80(i), DEFAULT_AMOUNT, sender);
        }

        MTypes.OrderHint[] memory _orderHintArray = new MTypes.OrderHint[](2);
        _orderHintArray[0] = MTypes.OrderHint({hintId: 100, creationTime: 0}); //hintId = 100 was matched earlier
        _orderHintArray[1] = MTypes.OrderHint({hintId: 101, creationTime: 0});
        depositEthAndPrank(receiver, DEFAULT_PRICE.mulU80(DEFAULT_AMOUNT * 2) * 6);
        startGas = gasleft();
        diamond.createLimitShort(asset, DEFAULT_PRICE, DEFAULT_AMOUNT * 2, _orderHintArray, shortHintArrayStorage, initialCR);
        gasUsed = startGas - gasleft();

        // @dev If findOrderHintId returned HEAD instead of startingShortId, the gas would be alot crazier
        // 194771 vs 409255
        emit log_named_uint("gasUsed", gasUsed);
    }
