...
contract ShortsTest is OBFixture {
    ...
    function primaryExitShort(bool isFullExit) public {
        uint256 ercMintedFromAsk = DEFAULT_AMOUNT.mulU88(3 ether);
        if (isFullExit) {
            ...
            assertEq(diamond.getAssetStruct(asset).ercDebt, DEFAULT_AMOUNT.mulU88(2 ether) + ercMintedFromAsk);
            ...
            assertEq(diamond.getAssetStruct(asset).ercDebt, DEFAULT_AMOUNT.mulU88(2.5 ether) + ercMintedFromAsk);
        }
    }

    function exitShortFromWallet(bool isFullExit) public {
        uint88 ercMinted = DEFAULT_AMOUNT.mulU88(3 ether);
        if (isFullExit) {
            ...
            assertEq(token.balanceOf(sender), DEFAULT_AMOUNT * 2);
            assertEq(diamond.getAssetStruct(asset).ercDebt, DEFAULT_AMOUNT * 2 + ercMinted);
        } else {
            ...
            s.ethEscrowed = 0;
            assertEq(diamond.getAssetStruct(asset).ercDebt, DEFAULT_AMOUNT * 3 - (DEFAULT_AMOUNT / 2) + ercMinted);
        }
    ...
    function exitShortWithERC(bool isFullExit) public {
        uint256 ercMintedFromMint = DEFAULT_AMOUNT.mulU88(3 ether);
        if (isFullExit) {
            exitShortErcEscrowed(C.SHORT_STARTING_ID + 1, DEFAULT_AMOUNT, sender);
            assertEq(getShortRecordCount(sender), 2);
            s.ethEscrowed = SHORT2_COLLATERAL;
            s.ercEscrowed = DEFAULT_AMOUNT * 2;
            assertEq(diamond.getAssetStruct(asset).ercDebt, DEFAULT_AMOUNT * 2 + ercMintedFromMint);
        } else {
            exitShortErcEscrowed(C.SHORT_STARTING_ID + 1, DEFAULT_AMOUNT / 2, sender);
            assertEq(getShortRecordCount(sender), 3);
            s.ercEscrowed = DEFAULT_AMOUNT * 3 - (DEFAULT_AMOUNT / 2);
            assertEq(diamond.getAssetStruct(asset).ercDebt, DEFAULT_AMOUNT * 3 - (DEFAULT_AMOUNT / 2) + ercMintedFromMint);
        }
        assertStruct(receiver, r);
        assertStruct(sender, s);
        assertStruct(extra, e);
    ...
    function checkCombineShortsx3() public view {
    ...
    function test_TryFullExitShortNotEnoughAsksForBuyBack() public {
        uint256 ercMintedFromAsk = DEFAULT_AMOUNT / 2;
        makeShorts();
    ...
    function test_ShortLinkingCancelShortOrderWhenRecordCancelled() public {
        fundLimitBidOpt(DEFAULT_PRICE + 1, DEFAULT_AMOUNT, receiver); //100
        fundLimitShortOpt(DEFAULT_PRICE + 1, DEFAULT_AMOUNT.mulU88(3 ether), sender); //101
