...
contract OwnerTest is OBFixture {
    ...
    function test_Revert_SetDiscountPenaltyFee() public {
        vm.expectRevert(Errors.NotOwnerOrAdmin.selector);
        diamond.setDiscountPenaltyFee(asset, 100);

        vm.startPrank(owner);
        vm.expectRevert("Can't be zero");
        diamond.setDiscountPenaltyFee(asset, 0);
        vm.expectRevert("above 10.0%");
        diamond.setDiscountPenaltyFee(asset, 1000 + 1);
    }

    function test_Revert_SetDiscountMultiplier() public {
        vm.expectRevert(Errors.NotOwnerOrAdmin.selector);
        diamond.setDiscountMultiplier(asset, 100);

        vm.startPrank(owner);
        vm.expectRevert("Can't be zero");
        diamond.setDiscountMultiplier(asset, 0);
        vm.expectRevert("above 65534");
        diamond.setDiscountMultiplier(asset, type(uint16).max);
    }
    ...
    function test_SetDiscountPenaltyFee() public {
        assertEq(diamond.getAssetStruct(asset).discountPenaltyFee, 10);
        vm.prank(owner);
        diamond.setDiscountPenaltyFee(asset, 11);
        assertEq(diamond.getAssetStruct(asset).discountPenaltyFee, 11);
    }

    function test_SetDiscountMultiplier() public {
        assertEq(diamond.getAssetStruct(asset).discountMultiplier, 10000);
        vm.prank(owner);
        diamond.setDiscountMultiplier(asset, 5001);
        assertEq(diamond.getAssetStruct(asset).discountMultiplier, 5001);
    }
    ...
    function test_CreateMarket() public {
        ...
        a.minShortErc = 2000;
        a.recoveryCR = 150;
        a.discountPenaltyFee = 10;
        a.discountMultiplier = 10000;
    ...
    function test_DittoMintToTreasury() public view {
