...
contract ShortsRevertTest is OBFixture {
    ...
    function test_Revert_CombinedShortBelowCRThreshold() public {
        fundLimitBidOpt(DEFAULT_PRICE, DEFAULT_AMOUNT, receiver);
        fundLimitShortOpt(DEFAULT_PRICE, DEFAULT_AMOUNT, sender);
        fundLimitBidOpt(DEFAULT_PRICE, DEFAULT_AMOUNT, receiver);
        fundLimitShortOpt(DEFAULT_PRICE, DEFAULT_AMOUNT, sender);
        ...
        STypes.ShortRecord memory shortRecord = diamond.getShortRecord(asset, sender, C.SHORT_STARTING_ID);

        _setETH(1500 ether);

        assertGe(diamond.getCollateralRatio(asset, shortRecord), C.MAX_REDEMPTION_CR);
        assertLt(diamond.getCollateralRatio(asset, shortRecord), diamond.getAssetNormalizedStruct(asset).liquidationCR);
        assertLt(C.MAX_REDEMPTION_CR, diamond.getAssetNormalizedStruct(asset).liquidationCR);

        vm.prank(sender);
        vm.expectRevert(Errors.CombinedShortBelowCRThreshold.selector);
        diamond.combineShorts(asset, shortIds, shortOrderIds);
    }

    function test_Revert_CombinedShortBelowMaxRedemptionCR() public {
        fundLimitBidOpt(DEFAULT_PRICE, DEFAULT_AMOUNT, receiver);
        fundLimitShortOpt(DEFAULT_PRICE, DEFAULT_AMOUNT, sender);
        fundLimitBidOpt(DEFAULT_PRICE, DEFAULT_AMOUNT, receiver);
        fundLimitShortOpt(DEFAULT_PRICE, DEFAULT_AMOUNT, sender);
        ...
        testFacet.setLiquidationCRT(asset, 190);

        STypes.ShortRecord memory shortRecord = diamond.getShortRecord(asset, sender, C.SHORT_STARTING_ID);
        _setETH(1300 ether);
        assertGe(diamond.getCollateralRatio(asset, shortRecord), diamond.getAssetNormalizedStruct(asset).liquidationCR);
        assertLt(diamond.getCollateralRatio(asset, shortRecord), C.MAX_REDEMPTION_CR);
        assertLt(diamond.getAssetNormalizedStruct(asset).liquidationCR, C.MAX_REDEMPTION_CR);
    ...
    function test_Revert_InvalidInitialCR_InitialCRLtInitialCR() public {
        ...
        // 100 wei = C.BID_CR as uint16
        uint16 initialCR = diamond.getAssetStruct(asset).initialCR - 100 wei - 1 wei;
