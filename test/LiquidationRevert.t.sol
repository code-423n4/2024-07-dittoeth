...
contract LiquidationRevertTest is LiquidationHelper {
    ...
    function test_RevertBlackSwan() public {
        ...
        diamond.setErcDebt(asset, sender, C.SHORT_STARTING_ID, 15000 ether);
    ...
    function test_RevertBlackSwan_IncludeTapp() public {
        prepareAsk({askPrice: DEFAULT_PRICE, askAmount: DEFAULT_AMOUNT});
        fundLimitAskOpt(DEFAULT_PRICE, DEFAULT_AMOUNT, receiver);

        //mock give tappSR ercDebt + increase Asset.ercDebt
        diamond.setErcDebt(asset, tapp, C.SHORT_STARTING_ID, DEFAULT_AMOUNT);
        diamond.addErcDebtAsset(asset, DEFAULT_AMOUNT);

        _setETH(730 ether); // c-ratio 1.095
        // Mock give the sender's SR 15000 ercDebt without increasing Asset.ercDebt
        diamond.setErcDebt(asset, sender, C.SHORT_STARTING_ID, 15000 ether);
        // 5000 minted from short match, 10000 minted from 2 fundLimitAskOpt calls, 5000 in tappSR
        assertEq(diamond.getAssetStruct(asset).ercDebt, 20000 ether);
