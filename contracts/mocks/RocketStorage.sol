...
contract RocketStorage {
    ...
    function setReth(address addr) external {
        addressStorage[keccak256(abi.encodePacked("contract.address", "rocketTokenRETH"))] = addr;
    }

    function setDeposit(address addr) external {
        addressStorage[keccak256(abi.encodePacked("contract.address", "rocketDepositPool"))] = addr;
    }
}
