...
contract GasRedemptionFixture is GasHelper {
    ...
    function gasRedemptionBeforeProposalAssert(address redeemer, address shorter) public view {
    ...
    function gasRedemptionProposalAssert(address redeemer, address shorter, uint8 numRedemptions) public view {
    ...
    function gasRedemptionBeforeDisputeAssert(address redeemer, uint8 numRedemptions) public view {
        address sstore2Pointer = diamond.getAssetUserStruct(asset, redeemer).SSTORE2Pointer;
        uint8 slateLength = diamond.getAssetUserStruct(asset, redeemer).slateLength;
        (,,,, MTypes.ProposalData[] memory decodedProposalData) = LibBytes.readProposalData(sstore2Pointer, slateLength);
    ...
    function gasRedemptionDisputeAssert(address redeemer, bool isAllIncorrect, uint8 initialLength) public view {
        address sstore2Pointer = diamond.getAssetUserStruct(asset, redeemer).SSTORE2Pointer;
        uint8 slateLength = diamond.getAssetUserStruct(asset, redeemer).slateLength;
        if (isAllIncorrect) {
    ...
    function gasRedemptionAfterRedeemingAssert(address redeemer) public view {
        address sstore2Pointer = diamond.getAssetUserStruct(asset, redeemer).SSTORE2Pointer;
        assertEq(sstore2Pointer, address(0));
    }
}
...
contract GasProposalTest is GasRedemptionFixture {
    ...
    function test_GasProposingOneSR() public {
        ...
        diamond.proposeRedemption(
            _asset, proposalInputs, DEFAULT_AMOUNT * _numRedemptions, MAX_REDEMPTION_FEE, MAX_REDEMPTION_DEADLINE
        );
        stopMeasuringGas();
...
contract GasDisputeTest is GasRedemptionFixture {
    function setUp() public override {
        ...
        ob.setETH(1000 ether);
        testFacet.setOracleTimeAndPrice(asset, testFacet.getOraclePriceT(asset));
        assertEq(ob.getShortRecordCount(sender), 101);

        //for tests involving closed SR
        ob.depositUsd(sender, DEFAULT_AMOUNT);
    }

    function preDisputeProposal(address asset, address redeemer, address shorter, uint8 numRedemptions, bool isPartial) public {
        ...
        uint88 redemptionAmt;

        if (isPartial) redemptionAmt = DEFAULT_AMOUNT * (numRedemptions - 1) + DEFAULT_AMOUNT / 2;
        else redemptionAmt = DEFAULT_AMOUNT * numRedemptions;

        vm.prank(redeemer);
        diamond.proposeRedemption(asset, proposalInputs, redemptionAmt, MAX_REDEMPTION_FEE, MAX_REDEMPTION_DEADLINE);
    ...
    function test_GasDisputeOneSR_ClosedSR() public {
        address _asset = asset;
        address _redeemer = receiver;
        address _shorter = sender;
        address _tapp = tapp;
        ...
        preDisputeProposal({asset: _asset, redeemer: _redeemer, shorter: _shorter, numRedemptions: _numRedemptions, isPartial: true});
        //close SR
        vm.prank(_shorter);
        diamond.exitShortErcEscrowed(_asset, C.SHORT_STARTING_ID, DEFAULT_AMOUNT / 2, 0);
        assertEq(ob.getShortRecordCount(sender), 100);
        STypes.ShortRecord memory tappSR = diamond.getShortRecord(asset, _tapp, C.SHORT_STARTING_ID);
        assertEq(tappSR.collateral, 0);
        assertEq(tappSR.ercDebt, 0);

        vm.prank(_disputer);
        startMeasuringGas("Redemption-DisputeOneSR-ClosedSR");
        diamond.disputeRedemption({
            asset: _asset,
            redeemer: _redeemer,
            incorrectIndex: 0,
        ...
        tappSR = diamond.getShortRecord(asset, _tapp, C.SHORT_STARTING_ID);
        assertGt(tappSR.collateral, 0);
        assertGt(tappSR.ercDebt, 0);
        gasRedemptionDisputeAssert({redeemer: _redeemer, isAllIncorrect: true, initialLength: _numRedemptions});
    }
...
contract GasClaimOneRedemptionTest is GasRedemptionFixture {
    function setUp() public override {
        ...
        vm.prank(_redeemer);
        diamond.proposeRedemption(
            asset, proposalInputs, DEFAULT_AMOUNT * _numRedemptions, MAX_REDEMPTION_FEE, MAX_REDEMPTION_DEADLINE
        );
...
contract GasClaimRemainingCollateralTest is GasRedemptionFixture {
    function setUp() public override {
        ...
        gasRedemptionBeforeProposalAssert({redeemer: _redeemer, shorter: _shorter});
        vm.prank(_redeemer);
        diamond.proposeRedemption(asset, proposalInputs, DEFAULT_AMOUNT, MAX_REDEMPTION_FEE, MAX_REDEMPTION_DEADLINE);
