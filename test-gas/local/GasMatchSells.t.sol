...
contract GasSellFixture is GasHelper {
    ...
    function gasAskMatchBidTestAsserts(bool isFullyMatched) public view {
    ...
    function gasShortMatchBidTestAsserts(bool isFullyMatched, uint256 startingShortId) public view {
        uint256 vault = VAULT.ONE;
        if (isFullyMatched) {
...
contract MatchingAskToBidAtDiscountTest is GasHelper {
    using U256 for uint256;
    ...
    function setUp() public override {
        ...
        ob.fundLimitBidOpt(DEFAULT_PRICE, ERCDEBTSEED, receiver);
        ob.fundLimitShortOpt(DEFAULT_PRICE, ERCDEBTSEED, receiver);
    ...
    function testGas_MatchAskToBid_MatchAtDiscount() public {
        ...
        diamond.createAsk(_asset, lowerPrice, ERCDEBTSEED, C.LIMIT_ORDER, orderHintArray);
        stopMeasuringGas();
        assertEq(ob.getBids().length, 0);
...
contract MatchingAskToBidDiscountBackToNormalTest is GasSellFixture {
    using U256 for uint256;
    ...
    function setUp() public override {
        ...
        ob.fundLimitShortOpt(DEFAULT_PRICE, ERCDEBTSEED, receiver);
        ob.fundLimitBidOpt(lowerPrice, ERCDEBTSEED, receiver);
        ob.fundLimitAskOpt(lowerPrice, ERCDEBTSEED, receiver);
        assertEq(diamond.getAssetStruct(asset).ercDebtRate, discountPenaltyFee);
        ob.fundLimitBidOpt(DEFAULT_PRICE, ERCDEBTSEED, receiver);
        assertEq(ob.getBids().length, 1);
    }
    ...
    function testGas_MatchAskToBid_MatchBackAtNormal() public {
        ...
        diamond.createAsk(_asset, DEFAULT_PRICE, ERCDEBTSEED, C.LIMIT_ORDER, orderHintArray);
        stopMeasuringGas();
        assertEq(ob.getBids().length, 0);
    }
}
