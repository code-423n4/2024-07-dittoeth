...
contract GasShortCollateralTest is GasShortFixture {
    ...
    function setUp() public override {
        ...
        // @dev Only affects last test in this contract related to recovery mode
        vm.startPrank(owner);
        diamond.setRecoveryCR(asset, 200);
        diamond.setInitialCR(asset, 50);
    }
    ...
    function testGas_DecreaseCollateral_BelowRecoveryCR() public matchShorts(1) {
        address _asset = asset;
        vm.prank(sender);
        startMeasuringGas("ShortRecord-DecreaseCollateral-RecoveryCR");
        diamond.decreaseCollateral(_asset, C.SHORT_STARTING_ID, DEFAULT_AMOUNT.mulU88(DEFAULT_PRICE));
        stopMeasuringGas();
        uint256 collateral = DEFAULT_PRICE.mulU80(DEFAULT_AMOUNT) * 6;
        assertEq(ob.getShortRecord(sender, C.SHORT_STARTING_ID).collateral, collateral - DEFAULT_AMOUNT.mulU88(DEFAULT_PRICE));
