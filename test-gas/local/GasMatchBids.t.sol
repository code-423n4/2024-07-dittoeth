...
contract GasBidFixture is GasHelper {
    ...
    function gasBidMatchAskTestAsserts(bool isFullyMatched) public view {
    ...
    function gasBidMatchShortTestAsserts(bool isFullyMatched, uint256 shortLen, uint256 startingShortId) public view {
        uint256 vault = VAULT.ONE;
        if (isFullyMatched) {
            assertEq(ob.getBids().length, 0);
        } else {
...
contract MatchingBidToAskAtDiscountTest is GasHelper {
    using U256 for uint256;
    ...
    function setUp() public override {
        ...
        ob.fundLimitBidOpt(DEFAULT_PRICE, ERCDEBTSEED, extra);
        ob.fundLimitShortOpt(DEFAULT_PRICE, ERCDEBTSEED, extra);
        ob.fundLimitAskOpt(lowerPrice, ERCDEBTSEED, sender);
        ob.depositEth(receiver, ERCDEBTSEED);
    ...
    function testGas_MatchBidToAsk_MatchAtDiscount() public {
        ...
        diamond.createBid(_asset, lowerPrice, ERCDEBTSEED, C.LIMIT_ORDER, orderHintArray, shortHintArray);
        stopMeasuringGas();
        assertEq(ob.getAsks().length, 0);
...
contract MatchingBidToAskDiscountBackToNormalTest is GasBidFixture {
    using U256 for uint256;
    ...
    function setUp() public override {
        ...
        ob.fundLimitBidOpt(DEFAULT_PRICE, ERCDEBTSEED, extra);
        ob.fundLimitShortOpt(DEFAULT_PRICE, ERCDEBTSEED, extra);
        ob.fundLimitBidOpt(lowerPrice, ERCDEBTSEED, receiver);
        ob.fundLimitAskOpt(lowerPrice, ERCDEBTSEED, sender);
        assertEq(diamond.getAssetStruct(asset).ercDebtRate, discountPenaltyFee);
        ob.fundLimitAskOpt(DEFAULT_PRICE, ERCDEBTSEED, sender);
        assertEq(ob.getAsks().length, 1);
    }
    ...
    function testGas_MatchBidToAsk_MatchBackAtNormal() public {
        ...
        diamond.createBid(_asset, DEFAULT_PRICE, ERCDEBTSEED, C.LIMIT_ORDER, orderHintArray, shortHintArray);
        stopMeasuringGas();
        assertEq(ob.getAsks().length, 0);
    }
}
