...
contract ExampleMigration0X is MigrationHelper {
    ...
    // A Batch can be sent to the safe wallet programmatically. Command to run script is:
    // scripts/forge-safe-batch.sh ExampleMigration0X runBatch false
    // First argument is the script name, second is the function to call, third is whether to send the batch
    // Function must accept a bool as an argument
    function runBatch(bool send_) external {
        bytes memory empty = new bytes(0);
        addToBatch(_dittoDeployer, 0.1 ether, empty);
        addToBatch(_dittoDeployer, 0.1 ether, empty);
        addToBatch(_dittoDeployer, 0.1 ether, empty);

        // Execute batch
        executeBatch(_safeWallet, send_);
    }
}
